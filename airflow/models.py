...
class TaskInstance(Base):
    ...
    def are_dependencies_met(
        ...
        for dep_status in self.get_failed_dep_statuses(
            ...
            if verbose:
                logging.warning(
                    "Dependencies not met for %s in context %s, dependency '%s' FAILED: "
                    "%s",
                    self, dep_context.__class__, dep_status.dep_name, dep_status.reason)
            return False
        if verbose:
            logging.info("Dependencies all met for %s in context %s",
                         self, dep_context.__class__)
        return True
...
class DAG(BaseDag, LoggingMixin):
    ...
    @provide_session
    def get_running_dagruns(self, session=None):
        """
        Return the dagruns that are currently running
        """
        return (
            session.query(DagRun)
            .filter(
    ...
    def active_runs(self):
        ...
        session = settings.Session()
        active_dates = []
        active_runs = self.get_running_dagruns(session)
