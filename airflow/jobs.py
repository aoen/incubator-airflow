...
class SchedulerJob(BaseJob):
    ...
    @provide_session
    def _cleanup_task_instances_up_for_retry(self, simple_dag_bag, session=None):
        """
        For all DAG IDs in the SimpleDagBag, look for task instances in the
        UP_FOR_RETRY state and set them to FAILED if the DagRun is not running.

        :param simple_dag_bag: TaskInstances associated with DAGs in the
        simple_dag_bag and in the UP_FOR_RETRY state will be examined.
        :type simple_dag_bag: SimpleDagBag
        """

        # Get all the queued task instances
        task_instances_up_for_retry = (
            session
            .query(models.TaskInstance)
            .filter(models.TaskInstance.dag_id.in_(simple_dag_bag.dag_ids))
            .filter(models.TaskInstance.state.in_([State.UP_FOR_RETRY]))
            .all()
        )
        """:type: list[TaskInstance]"""

        for task_instance in task_instances_up_for_retry:
            dag_run = DagRun.get_run(session,
            ...
            if dag_run is None:
                ...
                # Since the DagRun isn't running, the task shouldn't be
                # up for retry.
                self.logger.warn("Setting %s to %s as the state of "
                                 "%s is not %s",
                                 task_instance,
                                 State.FAILED,
                                 dag_run,
    ...
    def _execute_helper(self, processor_manager):
        ...
        while (datetime.now() - execute_start_time).total_seconds() < \
            ...
            if len(simple_dags) > 0:
                ...
                self._cleanup_task_instances_up_for_retry(simple_dag_bag)
