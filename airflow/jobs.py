...
class SchedulerJob(BaseJob):
    ...
    def _change_state_for_tis_without_dagrun(self,
        ...
        task_instances_up_for_retry = (
            ...
            .filter(models.TaskInstance.state.in_([old_state]))
            .all()
        )
        ...
        for task_instance in task_instances_up_for_retry:
            ...
            if dag_run is None:
                ...
                self.logger.warn("Setting %s to state=%s as the state of "
                                 "%s is not %s",
                                 task_instance,
                                 new_state,
                                 dag_run,
        ...
        while (datetime.now() - execute_start_time).total_seconds() < \
            ...
            if len(simple_dags) > 0:
                ...
                self._change_state_for_tis_without_dagrun(simple_dag_bag,
                                                          State.UP_FOR_RETRY,
                                                          State.FAILED)
                # If a task instance is queued, but the corresponding DAG run isn't
                # running, set the state to NONE so we don't try to re-run it.
                self._change_state_for_tis_without_dagrun(simple_dag_bag,
                                                          State.QUEUED,
                                                          State.NONE)
