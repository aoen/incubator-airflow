...
class HiveServer2Hook(BaseHook):
    ...
    def get_conn(self, schema=None):
        ...
        if owner_type.lower().strip() == "owner" and self.run_as_user:
            ...
            effective_user = self.run_as_user if self.run_as_user else (db.login if db.login else "anonymous")

        # For no auth, Impyla uses "PLAIN" whereas PyHive uses "NONE"
        if auth_mechanism == "NONE":
            auth_mechanism = "PLAIN"

        # Kerberos requires a db.login user that's the principal name.
        # The run_as_user should be required and its value used in hive.server2.proxy.user
        if configuration.get('core', 'security').upper() == 'KERBEROS':
        ...
        if auth_mechanism.upper() == "LDAP":
            ...
            if auth_mechanism.upper() == "KERBEROS":
            ...
            # [AIRBNB]
            # Custom change for Airbnb. Without authentication in HiveServer2, take the effective username instead.
            return connect(
                host=db.host,
                port=db.port,
                auth_mechanism=auth_mechanism,
