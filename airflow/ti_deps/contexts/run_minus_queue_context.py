...
class RunMinusQueueContext(BaseDepContext):
    ...
    This class is needed because the QueueContext has some slow queries that shouldn't be
    run twice per task instance execution cycle, so for efficiency we use:
    ...
    def get_ignoreable_deps(self, ti):
        ...
        run_deps = run_context.get_ignoreable_deps(ti)
        ...
        queue_deps = queue_context.get_ignoreable_deps(ti)

        return run_deps - queue_deps
