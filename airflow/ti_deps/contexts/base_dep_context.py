...
class BaseDepContext(object):
    ...
    def deps_for_ti(self, ti):
        """
        Get the context's dependencies for a given task instance.
    ...
    def get_ignoreable_deps(self, ti):
        """
        Get the ignoreable dependencies of the context unless the context specified that
        they should be ignored. By default we just return the task-specific dependencies
        for the task instance, but can be subclassed to return other ignoreable
    ...
    def get_nonignoreable_deps(self):
        """
        Get the dependencies for a given task that cannot be ignored when checking if
        dependencies are met, even by the context's 'ignore_all_deps' attribute. Can be
        subclassed to add additional non-ignoreable dependencies.
        """
        return set()
